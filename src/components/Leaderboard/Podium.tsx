...
export default function Podium({ topThree }: PodiumProps) {
    ...
    const PodiumCard = ({ entry, rank, height, color, icon, delay, shadow }: any) => (
        <motion.div
            ...
            className={`
                relative flex flex-col items-center group mb-4 
                ${rank === 1 ? 'order-1 md:order-2 scale-110 md:scale-100 z-10' : rank === 2 ? 'order-2 md:order-1' : 'order-3'}
                w-full md:w-auto
            `}
        >
            {/* Avatar / Crown info */}
            <div className="relative mb-6 md:mb-8 pt-4">
                <motion.div
                    ...
                    className={`w-28 h-28 md:w-32 md:h-32 rounded-[2rem] bg-gradient-to-br ${color} p-1 shadow-2xl relative z-10`}
                >
            ...
            {/* Podium Base - Adjusted for mobile stacking */}
            <div className={`
                ${height} w-40 md:w-44 bg-gradient-to-b from-white/10 to-transparent rounded-t-[2.5rem] border-t border-x border-white/10 
                flex flex-col items-center pt-8 md:pt-10 px-6 text-center group-hover:bg-white/15 transition-all duration-500 relative
            ...
            `}>
                ...
                <p className="relative z-10 text-white font-black text-xs md:text-base mb-2 truncate w-full tracking-tight">{entry.username || "Emperor"}</p>
                ...
                <div className="mt-auto pb-6 relative z-10">
                    <span className="text-4xl md:text-5xl font-black text-white/5 tracking-tighter italic">#{rank}</span>
                </div>
            </div>
        </motion.div>
    );
    ...
    return (
        <div className="flex flex-col md:flex-row items-center md:items-end justify-center gap-12 md:gap-12 mb-20 pt-16 relative w-full px-4 md:px-0">
            <PodiumCard
                entry={first}
                rank={1}
                height="h-56 md:h-80"
